...
export default function KidsEnglishTask() {
  ...
  // Step 6 state - moved to top level to fix React hooks order
  ...
  const getCorrectAnswerByStep = (step, index) => {
    ...
    const stepDataMap = {
      ...
      6: ['slowly', 'amazing', 'whisper', 'apron', 'never', 'suspicious'][index],
    };
  ...
  const finishTest = () => {
    // Don't call checkAnswers() here - we'll calculate the total directly
    let total = 0;
    ...
    for (let i = 0; i < 11; i++) {
      ...
      const userAnswer = step4[i] || '';
      if (Array.isArray(correctAnswer)) {
        if (correctAnswer.some(ans => normalizeAnswer(userAnswer) === normalizeAnswer(ans))) {
          total++;
        }
      } else {
        if (normalizeAnswer(userAnswer) === normalizeAnswer(correctAnswer)) {
